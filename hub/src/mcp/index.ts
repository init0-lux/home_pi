...
/**
 * A single MCP tool descriptor exposed to LLMs.
 * Follows the MCP (Model Context Protocol) tool schema convention.
 */
...
const tools: McpToolDefinition[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        onlineOnly: {
          type: "boolean",
          description:
            "If true, only return devices that are currently online.",
        },
      },
    },
  },

  {
    ...
    description:
      "Get the current ON/OFF state of a specific device (or all channels on a device).",
    inputSchema: {
      type: "object",
      properties: {
        deviceId: {
  ...
  {
    name: "get_room_state",
    description: "Get the current state of all devices in a specific room.",
    inputSchema: {
      type: "object",
      properties: {
        roomId: {
...

  {
    name: "set_room_state",
    description:
      "Turn all devices in a room ON or OFF simultaneously. Useful for controlling an entire room at once.",
    inputSchema: {
      type: "object",
      properties: {
        roomId: {
...

  {
    ...
    description:
      "Create a new time-based automation schedule. Can target a device or an entire room. Supports one-shot and repeating (cron) schedules.",
    inputSchema: {
      type: "object",
      properties: {
        name: {
        ...
        runAt: {
          type: "number",
          description:
            "Unix epoch timestamp (seconds) when the schedule should fire.",
        },
        repeatCron: {
          type: "string",
          description:
...

  {
    ...
    description:
      "Get a high-level summary of the hub status: MQTT connectivity, device counts, online/offline breakdown, and scheduler status.",
    inputSchema: {
      type: "object",
      properties: {},
    },
  ...
  try {
    switch (name) {
      ...
      case "set_device_state": {
        ...
        return text(
          `✅ ${deviceName} (channel ${channel}) turned ${state}. ${deliveryNote}`,
        );
      }
      ...
      case "toggle_device": {
        ...
        const updatedState = stateManager.toggleDeviceState(
          deviceId,
          channel,
          "automation",
        );
        ...
        const deviceName = device.name ?? deviceId;
        return text(
          `🔄 ${deviceName} (channel ${channel}) toggled → now ${updatedState.state}.`,
        );
      }
      ...
      case "set_room_state": {
        ...
        for (const device of devices) {
          const ok = mqttGateway.publishCommand(roomId, device.id, 0, state);
          if (ok) published++;
        }
      ...
      case "trigger_automation": {
        ...
        if (!automation.enabled) {
          return error(
            `Automation "${automation.name}" is disabled and cannot be triggered.`,
          );
        }
      ...
      case "list_schedules": {
        ...
        const lines = schedules.map((s) => {
          ...
          const repeat = s.repeatCron
            ? ` | repeats: ${s.repeatCron}`
            : " | one-shot";
          return `${status} ${s.name} (id: ${s.id}) | runs at: ${runAt} | target: ${target}${repeat}`;
        });
      ...
      case "create_schedule": {
        ...
        if (!deviceId && !roomId)
          return error("Either deviceId or roomId must be provided.");
        if (!runAt) return error("runAt (Unix epoch seconds) is required.");

        const nowSec = Math.floor(Date.now() / 1000);
        if (!repeatCron && runAt <= nowSec) {
          return error(
            "runAt must be a future timestamp for one-shot schedules.",
          );
        }
...
export async function mcpRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  log.info("MCP tool server registered");
   ...
   *   {
   *     "name": "set_device_state",
   *     "input": { "deviceId": "abc123", "state": "ON" }
   *   }
   *
   * This follows the MCP protocol's tools/call message shape.
   */
  fastify.post(
    "/mcp/tools/call",
    { preHandler: [requireMcpApiKey] },
   ...
   * POST /mcp/tools/:name
   *
   * Convenience endpoint — call a specific tool directly without wrapping
   * in a { name, input } envelope. Useful for direct HTTP integration.
   *
   * Request body is passed directly as the tool input.
   */
