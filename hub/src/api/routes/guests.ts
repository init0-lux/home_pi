...
const CheckinSchema = z.object({
  ...
  checkinTime: z.number().int().positive().optional(),
});
...
const CreateScheduleSchema = z.object({
  ...
  runAt:       z.number().int().positive("runAt must be a Unix epoch timestamp in seconds"),
  repeatCron:  z
    .string()
    .regex(
...
const CreateAutomationSchema = z.object({
  name: z.string().min(1).max(128),
  trigger: z.object({
    type:     z.enum(["schedule", "guest_checkin", "guest_checkout", "device_state", "manual"]),
    roomId:   z.string().uuid().optional(),
    deviceId: z.string().min(1).optional(),
...
export async function guestsRoutes(fastify: FastifyInstance): Promise<void> {
   ...
   * POST /guest/checkin
   *
   * PMS integration entry point. Records the guest, emits GUEST_CHECKIN →
   * automation engine turns on all devices in the room.
   *
   * Example:
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const checkinTime      = parsed.data.checkinTime ?? Math.floor(Date.now() / 1000);
      ...
      // If the room already has an active guest, auto-checkout them first
      const existing = db
      ...
      if (existing) {
        log.warn(
          { existingGuestId: existing.id, roomId },
          "Room already has an active guest — auto-checking them out",
        );
        db.prepare(
      ...
      // Emit → automation engine handles room activation
      ...
      return ok(
        reply,
        {
          guest: { id: guestId, name, roomId, checkinTime, active: true },
   ...
   * POST /guest/checkout
   *
   * Mark a guest as checked out. Emits GUEST_CHECKOUT → automation engine
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const checkoutTime   = parsed.data.checkoutTime ?? Math.floor(Date.now() / 1000);
      ...
      const guest = db
        .prepare<
          [string],
          { id: string; name: string | null; room_id: string | null; active: number }
        >("SELECT id, name, room_id, active FROM guests WHERE id = ?")
        .get(guestId);

      if (!guest) {
        return fail(reply, 404, `Guest not found: ${guestId}`, "GUEST_NOT_FOUND");
      }

      if (!guest.active) {
        return fail(reply, 409, "Guest is already checked out", "ALREADY_CHECKED_OUT");
      }
      ...
      log.info({ guestId, roomId: guest.room_id, checkoutTime }, "Guest checked out");
  ...
  /**
   * GET /guest/:id
   * Get a specific guest record (active or historical).
   */
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!guest) {
        return fail(reply, 404, `Guest not found: ${request.params.id}`, "GUEST_NOT_FOUND");
      }
  ...
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!s) {
        return fail(reply, 404, `Schedule not found: ${request.params.id}`, "SCHEDULE_NOT_FOUND");
      }
      return ok(reply, s);
    },
  );
   ...
   *
   * Example — lights ON at 7 AM every day:
   *   {
   *     "name": "Morning lights",
   *     "roomId": "uuid",
   *     "action": { "state": "ON" },
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (deviceId && !deviceRegistry.isKnownDevice(deviceId)) {
        return fail(reply, 404, `Device not found: ${deviceId}`, "DEVICE_NOT_FOUND");
      }
      if (roomId && !deviceRegistry.getRoom(roomId)) {
        return fail(reply, 404, `Room not found: ${roomId}`, "ROOM_NOT_FOUND");
      }
      if (!deviceId && !roomId) {
        return fail(reply, 400, "Either deviceId or roomId must be provided", "MISSING_TARGET");
      }

      try {
        const schedule = scheduler.createSchedule({ name, deviceId, roomId, action, runAt, repeatCron });

        log.info(
          { scheduleId: schedule.id, name, runAt, repeatCron, userId: request.authUser?.id },
  ...
  fastify.patch<{ Params: { id: string } }>(
    ...
    { preHandler: [requireAuth, requireRole("admin", "operator")] },
    async (request, reply) => {
      if (!scheduler.getSchedule(request.params.id)) {
        return fail(reply, 404, `Schedule not found: ${request.params.id}`, "SCHEDULE_NOT_FOUND");
      }
      ...
      try {
        const updated = scheduler.updateSchedule(request.params.id, parsed.data);
        return ok(reply, updated);
      } catch (err) {
        log.error({ err, scheduleId: request.params.id }, "Failed to update schedule");
  ...
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!automation) {
        return fail(reply, 404, `Automation not found: ${request.params.id}`, "AUTOMATION_NOT_FOUND");
      }
   ...
   *
   * Example — turn on room when guest checks in:
   *   {
   *     "name": "Guest checkin - Room 101",
   *     "trigger": { "type": "guest_checkin", "roomId": "uuid" },
   *     "actions": [{ "type": "set_room_state", "roomId": "uuid", "state": "ON" }]
   *   }
   */
  fastify.post(
    "/automations",
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        const automation = automationEngine.createAutomation({ name, trigger, actions });
        ...
        return fail(reply, 500, "Failed to create automation", "INTERNAL_ERROR");
      }
    },
  );
  ...
  fastify.patch<{ Params: { id: string } }>(
    ...
    { preHandler: [requireAuth, requireRole("admin", "operator")] },
    async (request, reply) => {
      if (!automationEngine.getAutomation(request.params.id)) {
        return fail(reply, 404, `Automation not found: ${request.params.id}`, "AUTOMATION_NOT_FOUND");
      }
      ...
      try {
        const updated = automationEngine.updateAutomation(request.params.id, parsed.data);
        return ok(reply, updated);
      } catch (err) {
        log.error({ err, automationId: request.params.id }, "Failed to update automation");
        return fail(reply, 500, "Failed to update automation", "INTERNAL_ERROR");
      }
    },
  );
   ...
   * Manually trigger an automation rule — useful for testing or one-off overrides.
   */
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!automation.enabled) {
        return fail(reply, 422, "Automation is disabled", "AUTOMATION_DISABLED");
      }

      // Emit the event so the engine's normal flow runs
      ...
      // Also dispatch actions synchronously so the caller sees the effect immediately
